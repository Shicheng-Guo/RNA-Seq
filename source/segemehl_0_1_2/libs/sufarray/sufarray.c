...
getChildintervalsArr(void *space, 
  ...
  if(child) {
    ...
    if(!checkdelim || s->seq->sequences[s->suftab[i]] != s->seq->delim) {
      c[count*2] = i;
      c[count*2+1] = i1-1;
      count++;
    }
  ...
  while(i1 < s->numofsuffixes-1 && isnextlIndex(s,i1) && !isupIndex(s,i1) 
      && s->chldtab[i1].val != 0) {
    i2 = s->chldtab[i1].val;
    if(!checkdelim || s->seq->sequences[s->suftab[i1]] != s->seq->delim) {
      c[count*2] = i1;
      c[count*2+1] = i2-1;
      count++;
    }
  ...
  if(child && (!checkdelim || s->seq->sequences[s->suftab[i1]] != s->seq->delim)) {
    c[count*2] = i1;
    c[count*2+1] = j;
    count++;
  }
...
getChildintervals(void *space, 
  ...
  if(child) {
    ...
    if(!checkdelim || s->seq->sequences[i] != s->seq->delim) {
  ...
  while(i1 < s->numofsuffixes-1 && isnextlIndex(s,i1) && !isupIndex(s,i1) 
      && s->chldtab[i1].val != 0) {
    i2 = s->chldtab[i1].val;
    if(!checkdelim || s->seq->sequences[i1] != s->seq->delim) {
      addinterval(space, c, i1, i2-1);
    }
    i1 = i2;
  }

  if(child && (!checkdelim || s->seq->sequences[i1] != s->seq->delim)) {

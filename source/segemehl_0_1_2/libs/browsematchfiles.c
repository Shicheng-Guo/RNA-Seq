...
bl_matchfileGetInfo(matchfileView_t *view) {
  ...
  shadowed = newshadowedwin(32, 65, 9, 12);
  win = dershadowedwin(shadowed, 30, 62, 1, 1);
  ...
  if(view->curframe && view->curframe->cs) { 
    ...
    bl_matchfileTest(NULL, 0, 0, view->curframe->start+curptr, 
        &view->curframe->cs[curptr], view->curframe->ref[curptr], view->stats, 1, NULL);
    ...
    for(j=0,k=0; j < 5; j++) {
      ...
      if(ntcounts[(int)NT[j]]) {
        ...
        mvwprintw(win, 7+k, 52, "%.1f", sqrt(ntvarpos[(int)NT[j]]));
    ...
    mvwprintw(win, 7+k+6, 2, "P(V)=%.2f, P(NV)=%.2f",  
        log((double)view->stats->X/view->stats->N),
        log((double)view->stats->P/view->stats->N));

    mvwprintw(win, 7+k+7, 2, "P(V)*P(D|V)=%.2f, P(NV)*P(D|NV)=%.2f, S=%.2f",
        cs[curptr].p_refx, cs[curptr].p_ref, logadd(cs[curptr].p_ref, 
          cs[curptr].p_refx));

    mvwprintw(win, 7+k+8, 2, "P(V)*P(D|V)=%.2f, P(NV)*P(D|NV)=%.2f, S=%.2f",
        cs[curptr].p_consx, cs[curptr].p_cons, logadd(cs[curptr].p_cons, 
          cs[curptr].p_consx));
  ...
  for(k=0, i=0; i < view->curframe->width; i++) {
    ...
    if(view->curframe->ref) { 
      ...
      bl_matchfileTest(space, 0, 0, view->curframe->start+i, &view->curframe->cs[i], 
          view->curframe->ref[i], view->stats, 0, NULL);
...
bl_matchfileInitView(void *space, matchfile_t *file, 
    matchfileSampleStats_t *stats, fasta_t *set, Uint fw)
{

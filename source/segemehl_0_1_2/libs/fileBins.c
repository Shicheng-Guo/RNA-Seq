...
void
bl_fileBinsUnixSort (void *space, bl_fileBins_t *fb, const char *fldstr)
{
    Uint i;
    int ret;
    char *filename;
    ...
    for(i=0; i < fb->noofbins; i++) {
      ...
      ret = bl_UnixSort(space, filename, fldstr);
    }
...
void 
bl_fileBinDomainsUnixSort (void *space, bl_fileBinDomains_t *dms, const char *fldstr) 
{
    ...
    unsigned char remove) 
{
  ...
  for(i=0; i < dms->noofdomains; i++) {
    ...
    bl_UnixSortMerge(space, filenames, dms->domain[i].bins.noofbins, fldstr, newname);

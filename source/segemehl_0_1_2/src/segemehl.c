...
int main(int argc, char** argv) {
  ...
  if (!manopt_isset(&optset, 'F', "bisulfite")){
    ...
    if ((!manopt_isset(&optset, 'i', NULL) && !manopt_isset(&optset, 'x', NULL) &&
        ...
        (manopt_isset(&optset, 'i', NULL) && manopt_isset(&optset, 'x', NULL)) ||
        (manopt_isset(&optset, 'j', NULL) && manopt_isset(&optset, 'y', NULL))){
      manopt_help(&optset, "please give index filename using (-i XOR -x) AND/OR (-j XOR -y) option\n");
    } else if(unflagged->noofvalues > 1) { 
      manopt_help(&optset, "unknown argument(s)\n");
    }
    ...
    if (manopt_isset(&optset, 'q', "query")){
      if (info.bestonly){
        ...
        if (!manopt_isset(&optset, 'o', "outfile") && !manopt_isset(&optset, 'B', "filebins")){
          manopt_help(&optset, "bisulfite mapping with two runs and best-only may not be used \
  ...
  for (k = 0; k < 2; k++){
    ...
    if(!info.bisulfiteprotocol){
      ...
      if (k == 0){
        ...
        /* bisulfite binning in case of bestonly and two matching runs */
    ...
    if(info.queryfilename) {
      ...
      if(info.softclip3Prime) {
        ...
        clipseq3 = ALLOCMEMORY(space, NULL, char, clipseqlen3 + 1);
        memmove(clipseq3, info.softclip3Prime, clipseqlen3);
        clipseq3[clipseqlen3] = 0;
        info.minclipscr3 = ceil((((float)info.softclip3PrimeLen)*info.clipacc)/100);
        //info.minclipscr3 = 5;
      }
      
      if (info.polyA) {
        info.polyAlen = MIN(50, info.reads->maxlen);
        clipseq3 = ALLOCMEMORY(space, clipseq3, char, clipseqlen3+info.polyAlen+1);
        memset(&clipseq3[clipseqlen3], 'A', info.polyAlen);
      ...
      if(info.softclip5Prime) {
        info.minclipscr5 = ceil((((float)info.softclip5PrimeLen)*info.clipacc)/100);
      }
  ...
  if (info.bisulfitemerging){
    ...
    se_mergeBisulfiteBins(space, info.bins, reads, info.dev, bins, 1);
  ...
  if (info.outfile && !info.bins) {
    ...
    if(info.order) {
      ...
      /* sort file */
      bl_UnixSort(space, info.outfile, SORT[info.rep_type]);
 ...
 // if(info.splitdev != NULL) fclose(info.splitdev);
  ...
  if(info.bins) {
    ...
    if(info.order) {
      bl_fileBinDomainsUnixSort(space, info.bins, SORTBIN[info.rep_type]);
    }
  ...
  if(info.splitbins) {
    ...
    if(info.order) {
      ...
      bl_fileBinDomainsUnixSort(space, info.splitbins, "-t $'\\t' -k9,9n");
    }

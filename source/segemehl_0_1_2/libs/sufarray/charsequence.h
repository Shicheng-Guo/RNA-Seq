...
static inline void bl_convertBisulfite(char *seqs[], Uint len, Uint bisulfite, Uint seed) {
  if (seed){
    /* bisulfite or PARCLIP in run 1 */
    if (bisulfite % 2 == 1){
      //fprintf(stderr,"seed conv of reads: C --> T\n");
      strconvert(seqs[0], len, 'C', 'T');
      strconvert(seqs[1], len, 'C', 'T');
    }
    /* bisulfite or PARCLIP in run 2 */
    if (bisulfite % 2 == 0){
      //fprintf(stderr,"seed conv of reads: G --> A\n");
      strconvert(seqs[0], len, 'G', 'A');
      strconvert(seqs[1], len, 'G', 'A');
    }
  }
  else {   

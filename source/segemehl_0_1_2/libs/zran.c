...
struct gzidxfile*
bl_initgzidxfile(FILE *fp, struct access *index, off_t offset)
{
  ...
  file->fp = fp;
  file->index = index;
  file->buf = calloc(SPAN+1, sizeof(unsigned char));
  file->buf[SPAN] = 0;
  file->curap = offset;
  file->len = 0;
  file->pos = file->buf;
...
bl_getgzidxc (struct gzidxfile *f)
{
  ...
  if(f->len == 0 || f->pos - f->buf >= f->len) {
    ...
    memset(f->buf, 0, SPAN+1);
    f->curap += f->pos - f->buf;
    f->len = extract(f->fp, f->index, f->curap, (unsigned char*) f->buf, SPAN);

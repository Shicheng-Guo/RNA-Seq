...
do
	...
	id=`echo $fastq | perl -ane '@L=split(/\//,$F[0]);foreach(@L){if(/\.bam/){($id)=split(/\./,$_);print "$id\n";}}'`

	#sam2bed.pl -i $fastq -b -o $MAP/$id.bed
	#gzip $MAP/$id.bed
	...
	zcat $MAP/$id.bed.gz | sort -k 6,6 -k 1,1 -k 2n,2 -k 3n,3 > $MAP/$id.bed.sorted
	...
	do
		...
		blockbuster.x -format 1 -distance 50 -minBlockHeight 10 -minClusterHeight 10 -scale 0.5 -chr $chr -print 2 -blockHeight rel $MAP/$id.bed.sorted >> $CLUSTER/$id.map.clusters
	done
	...
	#perl -an -F'/\s+/' -e 'if($_=~/^>/) { $isSig=0; $size=($F[3]-$F[2])+1; if($size<=500 && $F[10]!~/exon/) { print $_; $isSig=1; } } elsif($isSig) { print $_; }' $CLUSTER/$id.map.clusters.flagged >> $CLUSTER/$id.map.clusters.flagged.sig
	...
	#perl -an -F'/\s+/' -e 'if($_=~/^>/) { $isSig=0; $size=($F[3]-$F[2])+1; if($size<=500 && $F[10]!~/exon/ && $F[9]!~/n\/a/) { print $_; $isSig=1; } } elsif($isSig) { print $_; }' $CLUSTER/$id.map.clusters.flagged >> $CLUSTER/$id.map.clusters.flagged.sig.anno
	...
	perl -e 'print "#id\tmiRNA\tsnoRNA\ttRNA\tsnRNA\tscRNA\tyRNA\tpiRNA\tOthers\tUnannotated\tTotal\n";' >> $CLUSTER/bgAnnoDist.out
	grep "^>" $CLUSTER/$id.map.clusters.flagged | perl -an -F'/\s+/' -e '$total++; if($F[9]=~/miRNA/) { $mirna++ } elsif($F[9]=~/snoRNA/) { $snorna++; } elsif($F[9]=~/tRNA/) { $trna++; } elsif($F[9]=~/snRNA/) { $snrna++; } elsif($F[9]=~/scRNA/) { $scrna++; } elsif($F[9]=~/Y_RNA/) { $yrna++; } elsif($F[9]=~/piRNA/) { $pirna++; } elsif($F[9]=~/n\/a/) { $unannotated++; } else { $others++; } END { print "BG_'$id'_all\t$mirna\t$snorna\t$trna\t$snrna\t$scrna\t$yrna\t$pirna\t$others\t$unannotated\t$total\n"; }' >> $CLUSTER/bgAnnoDist.out
	grep "^>" $CLUSTER/$id.map.clusters.flagged.sig | perl -an -F'/\s+/' -e '$total++; if($F[9]=~/miRNA/) { $mirna++ } elsif($F[9]=~/snoRNA/) { $snorna++; } elsif($F[9]=~/tRNA/) { $trna++; } elsif($F[9]=~/snRNA/) { $snrna++; } elsif($F[9]=~/scRNA/) { $scrna++; } elsif($F[9]=~/Y_RNA/) { $yrna++; } elsif($F[9]=~/piRNA/) { $pirna++; } elsif($F[9]=~/n\/a/) { $unannotated++; } else { $others++; } END { print "BG_'$id'_sig\t$mirna\t$snorna\t$trna\t$snrna\t$scrna\t$yrna\t$pirna\t$others\t$unannotated\t$total\n"; }' >> $CLUSTER/bgAnnoDist.out
	grep "^>" $CLUSTER/$id.map.clusters.flagged.sig.anno | perl -an -F'/\s+/' -e '$total++; if($F[9]=~/miRNA/) { $mirna++ } elsif($F[9]=~/snoRNA/) { $snorna++; } elsif($F[9]=~/tRNA/) { $trna++; } elsif($F[9]=~/snRNA/) { $snrna++; } elsif($F[9]=~/scRNA/) { $scrna++; } elsif($F[9]=~/Y_RNA/) { $yrna++; } elsif($F[9]=~/piRNA/) { $pirna++; } elsif($F[9]=~/n\/a/) { $unannotated++; } else { $others++; } END { print "BG_'$id'_anno\t$mirna\t$snorna\t$trna\t$snrna\t$scrna\t$yrna\t$pirna\t$others\t$unannotated\t$total\n"; }' >> $CLUSTER/bgAnnoDist.out
	echo "Done...."

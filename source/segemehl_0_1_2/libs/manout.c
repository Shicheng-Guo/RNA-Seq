...
se_setMatches (void *space, gread_t *read, 
    gmatchlist_t *list, Uint maxedist, segemehl_t *nfo) {

  Uint i, k, n, noofmatepairs=0;
  gmatch_t *m = NULL, *cur;
  ...
  for(k=0; k <= 1; k++) {
    ...
    for(i=0; i < list->n[k]; i++) {
      ...
      if(cur->edist <= maxedist && !cur->skip) {
...
se_kdMatchListAdd(gmatchlist_t *list, 
  ...
  list->minedist = MIN(list->minedist, edist);
  match->scr = scr; 
  match->evalue = evalue;
...
gmatchlist_t*
bl_gmatchlistInit(void *space, int maxedist) {
...
getSAMTags(Gmap *map, Uint ustart, Uint uend, Uint uno, Alignment *al, 
    ...
    Uint nextidx, Uint nextpos, char nextflags, 
    Uint noofmatches, char pStatChr, char *meopstr, segemehl_t *nfo) {
  ...
  len = snprintf(NULL, 0, "NM:i:%d\tMD:Z:%s\tNH:i:%d", edist, md, noofmatches);
  tag = ALLOCMEMORY(space, NULL, char, len+1);
  snprintf(tag, len+1, "NM:i:%d\tMD:Z:%s\tNH:i:%d", edist, md, noofmatches);
  ptr += len;
...
reportMatch (void *space, Gmap *map, fasta_t *queries, 
  ...
  int i, k, j, l, u, seqlen, qrylen, matedesclen, matelen,
  mateseqlen, desclen;
  Uint off, clipoff=0, mateclipoff=0, seqstart=0, mateseqstart=0, flag=0, 
       mateflag=0, noofmatepairs=0, noofmatches=0, noofmatematches=0,
       lclip=0, rclip=0, matelclip=0, materclip=0, refdesclen=0, materefdesclen=0, 
       fraglen=0, quallen = 0;
      ...
      for(j = 0; j < read->n[k]; j++) {
        ...
        if (qual == NULL){
          ...
          if(match->previdx != -1 || match->nextidx != -1) {
            ...
            sprintstr(&list[QUAL], &qual[match->i], quallen);
          } else { 
            quallen = qrylen; 
        ...
        if (strandchr == '-') {
          meopstr = multieopstring(al, rclip, lclip, 0);
          cigar = cigarstring(al, rclip, lclip, (nfo->hardclip) ? 'H':'S', 0);

          if(match->previdx != -1 || match->nextidx != -1) {
            fraglen =match->j - match->i + 1; 
            tmp = charIUPACcomplement(space, &qry[match->i], fraglen); 
          } else {
            fraglen = qrylen;
          ...
          flag |= (1 << 4);
          mateflag |= (1 << 5);
          clipoff = (nfo->hardclip) ? rclip : 0;

        } else { 
          
          meopstr = multieopstring(al, lclip, rclip, 0);
          cigar = cigarstring(al, lclip, rclip, (nfo->hardclip) ? 'H':'S', 0);
          
          if(match->previdx != -1 || match->nextidx != -1) {
            tmp = ALLOCMEMORY(space, NULL, char, match->j - match->i + 2);
            memmove(tmp, &qry[match->i], match->j - match->i + 1); 
            tmp[match->j - match->i + 1] = 0;
            sprintstr(&list[QRY_SEQ], tmp, match->j - match->i + 1);
            FREEMEMORY(space, tmp);
          } else {
            sprintstr(&list[QRY_SEQ], qry, qrylen);
          }

          clipoff = (nfo->hardclip) ? lclip : 0;
        }
        ...
        sprintstr(&list[REF_SEQ], map->mseq->sequences+match->p, seqlen);
        ...
        tag = getSAMTags(map, off+match->i, off+match->j, match->fragno, match->al, 
            ...
            noofmatches, pStatChr, meopstr, nfo);
        ...
        if(bl_fastaHasMate(queries) && 
          ...
          if(se_kdMatchHasMates(&read->matches[k][j])) { 
            ...
            for (u=0; u < 4 && !mateReported; u++) {
              ...
              sprintstr(&list[MATE_REF_SEQ], 
                  map->mseq->sequences + mates[u].p, mateseqlen);
              //default quality string is *
              if (matequal == NULL){
              ...
              sprintUint(&list[MATE_NOOFMATCHES], match->noofmatematches);
              ...
              tag = getSAMTags(map, 1, matelen, 1, 
                  ...
                  -1, -1, 0, -1, -1, 0, noofmatematches, pStatChr, matemeopstr, nfo);

...
bl_matchfileIndexAddChrom(matchfileindex_t *index, char *chromname) {
  ...
  if(k == index->noofchroms) {
    ...
    index->matchstart = ALLOCMEMORY(space, index->matchstart, Uint, index->noofchroms+1);
    index->matchend = ALLOCMEMORY(space, index->matchend, Uint, index->noofchroms+1);
...
void bl_matchfileIndex (void *space, matchfile_t *file, fasta_t *set) {
  ...
  Uint buffersize = 1024, len = 0, curstart = 0, 
       ...
       lastend = 0, laststart = 0, p, u, s, q, id, allen=0,
  ...
  if (gzip) {
    ...
    gzf = bl_initgzidxfile(fp, index->gzindex, 0);
  } else {
    fp = fopen(filename, "r");
  }
...
//              if(strcmp(curchrom,"chr1")!=0) fprintf(stderr, "curchrom:%s (%d)\n", curchrom, id);

              ref = &bl_fastaGetSequence(set, id)[curstart-1];
              allen =strlen(aln);

              for(u=0, p=0, q=0; u < allen; u++) {
            ...
            if(curchrom) {
              ...
              if (k == index->noofchroms || !index->noofbins || startbin > curbin || lastchromidx != k) {
                fprintf(stderr, "adding new access point for chrom %s at off %llu \n", curchrom, off );
  ...
  if(matches) {
    ...
    index->bins[k][curbin].end = lastend;
  }
...
bl_matchfileRead(void *space, matchfile_t *file, char *chromname, 
  ...
  Uint buffersize=1024, row=0, startbin, 
       startsite, endbin, endsite, len=0, i=0, u=0, v=0, k=0, chridx=0, 
       curcnt=0, curalnlen, dellen = 0, curstart=0, curend=0, startcover=0, 
       curcover=0, acceptorpos=0, donorpos=0, xstart=0, xend = 0, xno = 0, 
       pluscnt = 0, minuscnt = 0,
       acceptorchridx, donorchridx, adjoint, donorcnt = 0, donorcntplus = 0,
       acceptorcntplus = 0, acceptorcnt = 0, readcnt=0, donorchrcnt=0, acceptorchrcnt=0,
       acceptorflg = 0, donorflg = 0, pnext = 0, rnextidx=0, curchromidx; 
  char *buffer = NULL, *delstring=NULL, *delquals=NULL, ch, 
       *curseq=NULL, *curqual=NULL, *curaln, *filename, strand,
       *acceptorchr = NULL, *donorchr = NULL, *rnext, *curchrom;
  unsigned char header = 1;
  int ret=0, edist=0, readlen=0;
  ...
  endbin = ((end) >> index->exp);
  k = bl_matchfileGetChromIndexNumber(index, chromname); 
 
   // while (index->bins[k][startbin].matches==0 && startbin < endbin) 
 //   startbin++;
  ...
  if (gzip) {  
    fp = fopen(filename, "rb");
    gzf = bl_initgzidxfile(fp, index->gzindex, index->bins[k][startbin].offset);
  } else {
    fp = fopen(filename, "r");
  ...
  while((ch = (gzip) ? bl_getgzidxc(gzf) : getc(fp)) != EOF && 
    ...
    if(ch == '\n' && len > 0) {
      ...
      if(!header) { 
        ...
        fprintf(stderr, "chromname:'%s', curchrom:'%s', buffer:%s\n", chromname,
        curchrom, buffer);
        ...
        if (curstart != curend && 
          ...
          fprintf(stderr, "within interval!\n");
          ...
          if(curcover < maxcover) { 
            ...
            readcnt++;
            if(donorchr) donorchrcnt++;
            if(acceptorchr) acceptorchrcnt++;
            if(strand=='-') minuscnt++;
            if(strand=='+') pluscnt++;
            ...
            if (curaln[startsite] != 'M' || curaln[endsite] != 'M') {
              startsiteset = 0;
            ...
            if(donorchr) {
              ...
              assert(donorchridx != -1);
              if(set) { 
                /*get the chromosome idx in the database*/
                for(chridx=0; chridx < set->noofseqs; chridx++) { 
                  if(strcmp(bl_fastaGetDescription(set,chridx),
                        index->chromnames[donorchridx])==0) {
                    break;
                  }
                }
                assert(chridx < set->noofseqs);
                donorchridx = chridx;
              }
              //if(i==0) donorcnt++;
            }
            ...
            if(acceptorchr) { 
              ...
              assert(acceptorchridx != -1);
              if(set) { 
                /*get the chromosome idx in the database*/
                for(chridx=0; chridx < set->noofseqs; chridx++) { 
                  if(strcmp(bl_fastaGetDescription(set,chridx),
                        index->chromnames[acceptorchridx])==0) {
                    break;
                  }
                }
                assert(chridx < set->noofseqs);
                acceptorchridx = chridx;
              }
              //if(i==0) acceptorcnt++;
            }
            ...
            if(rnext && rnext[0] == '=') { 
              rnextidx = k; 
            } else if(rnext && rnext[0] != '*') { 
              rnextidx =  bl_matchfileGetChromIndexNumber(index, rnext);
              assert(rnextidx != -1);
              fprintf(stderr, "rnext:%s; rnextidx:%d\n", rnext, rnextidx);
              if(set) { 
                /*get the chromosome idx in the database*/
                for(chridx=0; chridx < set->noofseqs; chridx++) { 
                  if(strcmp(bl_fastaGetDescription(set,chridx),
                        index->chromnames[rnextidx])==0) {
                    break;
                  }
                }
                assert(chridx < set->noofseqs);
            ...
            for(i = 0, u = 0; i < curalnlen; i++) {
              ...
              if(fields & MFREAD_SPLITS) { 
                if(donorchr && v < end-start+1 && (( i == 0 && strand == '+') || 
                      (i == curalnlen-1 && strand == '-'))) {
                  adjoint = -1;
                  ...
                  bl_matchfileAddSplit(space, cs, v, strand, 'A',
                      donorchridx, donorpos, adjoint, 
                      xstart, xend, row, xno, donorflg);
                  donorcnt++;
                  if(strand == '+') donorcntplus++;
                }

                if(acceptorchr && v < end-start+1 && (( i == 0 && strand == '-') || 
                      (i == curalnlen-1 && strand == '+'))) {
                  adjoint = -1;
                  ...
                  bl_matchfileAddSplit(space, cs, v, strand, 'D',
                      acceptorchridx, acceptorpos, adjoint, 
                      xstart, xend, row, xno, acceptorflg);
                  acceptorcnt++;
                  if(strand == '+') acceptorcntplus++;
                }
              }
              ...
              if(startsite == i && v < end-start+1) {
                cs[v].starts++;        
                if(rnext && rnext[0] != '*') {
                  bl_matchfileAddMate (space, &cs[v], rnextidx, pnext);
                }
              }
              ...
              if (eop != 'I') {
                ...
                if (delstring && fields & MFREAD_DELS) {
                  if (v < end-start+1) {
                    ...
                    if (fields & MFREAD_MCNT)
                    cs[v].dels[cs[v].noofdels].matchcnt = curcnt;
                ...
                if (eop != 'D') {
                  if(v < end-start+1) {
                    ...
                    cs[v].chars[cs[v].len-1] = curseq[u]; 
                    cs[v].chars[cs[v].len] = 0 ;
                    
                    
                    if(fields & MFREAD_QUAL) { 
                        cs[v].quals[cs[v].len-1] = curqual[u]; 
                    ...
                    cs[v].strands[cs[v].len-1] = strand;
                ...
                if(v < end-start+1) {
                  ...
                  if(fields & MFREAD_MCNT)
                  cs[v].matchcnt[cs[v].len-1] = curcnt;
                ...
                if(fields & MFREAD_DELS) { 
                  ...
                  if(fields & MFREAD_QUAL) { 
                    ...
                    delquals[dellen] = curqual[u];
                    delquals[dellen+1] = 0;
                  }
                  delstring[dellen] = curseq[u];
  ...
  fprintf(stderr, "exiting this function\n");
  ...
  for(k=0; k < index->noofchroms; k++) {
    ...
    for(i=0; i < index->noofbins[k]; i++) {
      ...
      if(index->bins[k][i].offset) {
        ...
        curstart = bl_matchfileGetStartPos(fields, 0);
...
bl_matchfileCopyCross(void *space, matchfileCross_t *xs, matchfileCross_t *cs) {
  ...
  if(cs->splits) {
    ...
    for(i=0; i < cs->noofsplits; i++) {
      if(xs->splits[i].edge > 113729595 && xs->splits[i].edge < 113729599)
        fprintf(stderr, "copied position %d for chridx:%d (type:%c), adjunct %d\n", xs->splits[i].edge, xs->splits[i].edgechridx, xs->splits[i].edgetype, xs->splits[i].adjoint);

      if(xs->splits[i].edge > 44116330 && xs->splits[i].edge < 44116334) 
        fprintf(stderr, "copied position %d for chridx:%d (type:%c), adjunct %d\n", xs->splits[i].edge, xs->splits[i].edgechridx, xs->splits[i].edgetype, xs->splits[i].adjoint);
    }
  }

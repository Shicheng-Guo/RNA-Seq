...
/*--------------------------------- chainovl ---------------------------------
 *    
 * @brief overlap of a fragment with chain
 * @author Steve Hoffmann 
 *   
 */
...
chainovl(branchChain_t *chain, branchfragment_t *f) {
  return ((Lint)chain->end - f->start)+1;
}
...
branchChain(void *space, Suffixarray *arr, matchstem_t **stems, char **seqs, 
  ...
  Uint i, j, u, q, 
    start, end, substart, 
    k=0, l, r, s, 
    c=0, c_prime=0, q_prime = 0, 
    x, maxocc=50;
  int maxovl = 12, maxgap = 22, bestscr;
  double maxevalue = .2, minentropy = 1.5, E, H;
  ...
  for (i = 0; i < len; i++) {
    for (u = 0; u < 2; u++) {
	  ...
	  //CHANGED: end position one position too far
          ...
          substart = arr->suftab[l];
        } else {
          start = i - branch->mat + 1;
          end = i;
          substart = arr->suftab[l] - branch->mat + 1;
        }
	    ...
	    //in order to account for minus strand hits correctly
            if( abs((Lint)f[j].substart - (Lint)substart) <
                abs((Lint)f[j].start - x) + maxovl) 
              break;
          }
  ...
  select = ALLOCMEMORY(space, NULL, unsigned char, k);
  memset(select, 0, k);
  ...
  for(i = 0; i < k; i++) {
    ...
    for (j = 0; j < c; j++) {
      //TO BE CHANGED? (please check)
      //do not need this loop since 
      //you just seek for predecessor 
      //of f[i] in chains
      for(q=i; q < k && f[i].start + maxgap > f[q].start; q++) {
        if(chainovl(&chains[j], &f[q]) < maxovl) {
          if (bestscr < chainscore(&chains[j], &f[q])) {
            bestscr = chainscore(&chains[j], &f[q]); 
            c_prime = j;
            f_prime = &f[q];
            q_prime = q;
          }
          if(bestscr == chainscore(&chains[j], &f[q]) && 
             minDistFragmentHits(arr, chains[j].f[chains[j].nooffragments-1], &f[q]) <
             minDistFragmentHits(arr, chains[j].f[chains[j].nooffragments-1], &f[q_prime])) {
            c_prime = j;
            f_prime = &f[q];
            q_prime = q;
          }
    ...
    if(f_prime) {
      chain(&chains[c_prime], f_prime); 
      select[q_prime] = 1;
    } 

    if (!select[i]) {
      c++;
      chains = ALLOCMEMORY(space, chains, branchChain_t, c);
      chains[c-1].nooffragments = 1;
      chains[c-1].f = 
        ALLOCMEMORY(space, NULL, branchfragment_t*, chains[c-1].nooffragments);
      chains[c-1].f[0] = &f[i];
      chains[c-1].score = f[i].score;
      chains[c-1].start = f[i].start;
      chains[c-1].end = f[i].end; 
    } 
  }
  ...
  for(i=0; i < noofchains; i++) {
    ...
    for(j=0; j < chains[i].nooffragments; j++) {
      ...
      for(q=chains[i].f[j]->branch->l; q <= chains[i].f[j]->branch->r; q++) {
        fprintf(dev,"%u (chr:%d), ",arr->suftab[q], 
            getMultiCharSeqIndex(arr->seq, &arr->seq->sequences[arr->suftab[q]]));
      }

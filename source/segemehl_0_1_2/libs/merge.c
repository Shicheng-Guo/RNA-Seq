 ...
 *  Author: $Author: steve $
 *  Date: $Date: 2011-07-22 18:14:45 +0200 (Fr, 22. Jul 2011) $
 *
 *  Id: $Id: merge.c 315 2011-07-22 16:14:45Z steve $
 ...
 *        paired-end reads (i.e. best pairing state) and minimal edit distance,
 *        returns -1 if first is better, 1 if second is better, 0 otherwise
 * @author Christian Otto
 *   
 */
...
se_mergeBisulfiteBins (void *space, bl_fileBinDomains_t *bsdomains, fasta_t **reads,
  ...
  for (i = 0; i < noofbins; i++){
    ...
    for (j = 0; j < reads[i]->noofseqs; j++){
       ...
       * NOTE: assumes that each file is already best-only,
       * hence only the next entry must be taken into account,
       * would require major adjustments in the merging if this
       * condition does not hold anymore (e.g. in a stand-alone tool)
       */
      for (k = 0; k < files.nooffiles; k++){
	while (1){
	  /* read next entry */
	  ...
	  /* 
           ...
           */
	  if (files.f[k].eof ||
              strncmp(curkey, files.f[k].entry->key,
                      MIN(curlen, files.f[k].entry->keylen)) != 0){
	    break;
	  }
	  
	  /* compare current with best match */
	  if (noofbest > 0){
	    cmp = bl_mergefilematchCompare(best[0], files.f[k].entry);
	  }
	  else {
	    cmp = 1;
	  }

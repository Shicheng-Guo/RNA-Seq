    ...
    char *C, Uint clen, Uint minclipscr) 
{
  ...
  if(getSWScore(al, edstscr, 0) >= minclipscr) {
...
char*
bl_seqclipFind3Prime (void *space, fasta_t *set, Uint samplesize, Uint fs, Uint ws)
{

  char *curseq, *curframe, *curwin;
  uint16_t *C;
  double curent = 0;
  Uint i, elem, j, size, curlen, curfs, *tab, curcode=0, bestcode=0, bestvalue=0;
  PairUint *B;

...
#### parse options ####
while getopts s:q:f:u:h ARG; do
	case "$ARG" in
...
echo -e "#reads (raw)\t#reads (after qualityCheck)\t#reads (for mapping)\t# reads (unpaired)\t#reads (unmapped)\t#reads (aligned 1 time)\t#reads (aligned >1 time)\talignment rate"
zless $MAPSTATFILE | perl -ane 'BEGIN { print "'$RAW_READS_COUNT'\t'$CLIPPED_READS_COUNT' ('$PER')"; } if($_=~/^[0-9\s]+/) { $_=~s/\;.*//g; chomp($_); $_=~s/\s+[a-zA-Z]+.*//g; print "\t$_"; } END { print "\n"; }'

...
{         
  ...
  fprintf(stderr, "getlistsL: memsetting list of %u elements\n", listlen);
  setbitarray(list, listlen, 0);
  
  fprintf(stderr, "getlistsL: iter from %lld down to 0\n", i);
  while(i >= 0) {
    end = i;
    if(i > 0) {
...
/*      printf("tmp %d\n", tmp);*/
  ...
  fprintf(stderr, "scanning A (%u elems)\n", len);
  for(i=0; i < len; i++) {
    if(dist[i] != (Uint)-1) {
      A[dist[i]] = i;    
    }
  ...
  fprintf(stderr, "scanning accdist (%u elems) (1)\n", maxdist);
  for(i=0; i < maxdist; i++) {
    if(accdist[i] > 0) {
  ...
  fprintf(stderr, "scanning accdist (%u elems) (2)\n", maxdist);
  for(i=0; i < maxdist; i++) {
    if (i==0) {
      j=0;
    } else {
    ...
    Uint listlen) 
{
 Uint i=0,
  ...
  fprintf(stderr, "getlistsS: memsetting list of %u elements\n", listlen);
  setbitarray(list, listlen, 0);

  fprintf(stderr, "getlistsS: iter up to %u\n", len);
  while(i < len) {
    start = i;
  ...
  fprintf(stderr, "getlistsS: scan A\n");
  for(i=0; i < len; i++) {
    if(dist[i] != (Uint)-1)
      A[dist[i]]=i;
  }

  fprintf(stderr, "getlistsS: set accidst\n");
  for(i=0; i < maxdist; i++) {
    if (accdist[i] == 0) fprintf(stderr, "getlistsS: i=%u accdist=0!!\n", i);
    else
    setbit(list, accdist[i]-1, 1);
  }
  ...
  fprintf(stderr,"getlistsS: exiting\n");
  return A;
}
    ...
    Uint listlen)
{
  ...
  fprintf(stderr, "sortlistS: allocating stuff\n");
  ...
  fprintf(stderr, "sortlistS: iterating %u elems", lenB);
  for(i=lenB-1; i > 0; i--) {
    rev[B[i]] = bcktright;
  ...
  fprintf(stderr, "sortlistS: looping %u elems", listlen);
  i=0;
  while (i < listlen) {
    j=i;
  ...
  fprintf(stderr, "sortlistS: iterating %u elems", len);
  for(i=0; i < len; i++) {
    bcktno = rev[i];
    if(bcktno > -1) {
        B[left[bcktno]] = i;
  ...
  fprintf(stderr, "sortlistsS: exiting happily!\n");
    ...
    Uint listlen)
{
  ...
  fprintf(stderr, "sortlistL: allocating stuff\n");
  rev =   ALLOCMEMORY(space, NULL, Lint, len);
  ...
  fprintf(stderr, "sortlistL: iterating %u elems", lenB);
  bcktleft=0;
  for(i=0; i < lenB; i++) {
  ...
  fprintf(stderr, "sortlistL: looping %u elems", listlen);
  i=0;
  while (i < listlen) {
  ...
  fprintf(stderr, "sortlistL: iterating %u elems", len);
  for(i=0; i < len; i++) {
    bcktno = rev[i];
    if(bcktno > -1) {
        B[right[bcktno]] = i;
  ...
  fprintf(stderr, "sortlistsL: exiting happily!\n");
  FREEMEMORY(space, rev);
    ...
    bitarray bckts) 
{
  ...
  *A,
  ...
  fprintf(stderr, "countingsortint: init buffers and A\n");
  buffer = ALLOCMEMORY(space, NULL, Uint, len);
  A = ALLOCMEMORY(space, NULL, Uint , len);

  fprintf(stderr, "setting buffer to zero\n");
  /*use buffer to count chars first*/
  memset(buffer, 0, sizeof(Uint)*sigma); 
  
  fprintf(stderr, "countsortint: scanning buffer (1 of 3)\n");
  for(i=0; i < len; i++) {
    resc = (Uint) (s[i] - min);
    buffer[resc]++;
  }
  ...
  fprintf(stderr, "countsortint: scanning buffer (2 of 3)\n");
  for(i=1; i < sigma; i++) {
    resc = buffer[i];
    buffer[i] = offset + buffer[i-1];
    offset = resc;
  ...
  fprintf(stderr, "countsortint: scanning buffer (3 of 3)\n");
  for(i=0; i < len; i++) {
    resc = (Uint) (s[i] - min);
    A[buffer[resc]] = i;
    buffer[resc]++;
  ...
  fprintf(stderr, "countsortint: scanning buffer (to set borders)\n");
  /*the bucket borders*/
  setbitarray(bckts, len, 0);
  for(i=0; i < sigma; i++) {
    setbit(bckts, buffer[i]-1, 1);
  ...
  fprintf(stderr, "countsortint: exiting\n");
  FREEMEMORY(space, buffer);
  return A;
}
    ...
    Uint Q) 
{
  ...
  fprintf(stderr, "setting bit array to zero\n");
  setbitarray(bckts, bucketno, 0);

  fprintf(stderr, "allocating space for buckets and buffers\n");
  ...
  fprintf(stderr, "substring sort ... ok\n"); 
    ...
    unsigned char Q) 
{
  ...
  *B;
  ...
  fprintf(stderr, "arrayB: allocating B with %u elements\n", lenB);
  B = ALLOCMEMORY(space, NULL, Uint, lenB);
  memset(B, 0, sizeof(Uint)*lenB);
  setbitarray(bcktsB, lenB, 0);
  
  fprintf(stderr, "arrayB: iterating to lenA=%u\n", lenA);
  for(i=0; i < lenA; i++) {
    if (getbit(cl, (Uint) A[i]) == type) { 
      B[j] = A[i];
      j++;
    }
    if (j > lenB) fprintf(stderr, "arrayB: j=%lld in B out of bounds!\n", j);
    /*copy bckt-borders*/
    if(getbit(bcktsA, (Uint)i) ==1 && j-1 >= 0)
      setbit(bcktsB, j-1, 1);
  }

  fprintf(stderr,"arrayB: exiting\n");
  return B;
}
    ...
    unsigned int Q) 
{
  ...
  fprintf(stderr, "reconstruct: init\n");
  conv = ALLOCMEMORY(space, NULL, Lint, lenB);
  
  fprintf(stderr, "reconstruct: iteration over %u elems\n", len);
  for(i=0; i < len; i++) {
  ...
  fprintf(stderr, "reconstruct: scan B (size: %u)\n", lenB);
  for(i=0; i < lenB; i++) {
    cur = B[i];
    B[i] = (Uint) conv[cur];
  }

  fprintf(stderr, "reconstruct: exit");
    ...
    bitarray cl) 
{
  ...
  fprintf(stderr, "aluruSuffixArrayL: initalizning arrays\n");
  count = ALLOCMEMORY(space, NULL, Lint, 255);
  sarray = ALLOCMEMORY(space, NULL, Uint, len);
  b = initbitarray(space, len);

  fprintf(stderr, "aluruSuffixArrayL: memsetting count\n");
  memset(count, 0, 255 *sizeof(Lint));

  fprintf(stderr, "aluruSuffixArrayL: setting b\n");
  setbitarray(b, len, 0);
  ...
  fprintf(stderr, "aluruSuffixArrayL: iteration\n");
  for(i=len-1; i >= 0; i--) {
    if (!getbit(b, (Uint) i)) {
      sarray[i] = B[j];
      setbit(b, (Uint) i, 1);
  ...
  FREEMEMORY(space, count);
  FREEMEMORY(space, b);
  fprintf(stderr, "aluruSuffixArrayL: exit ok\n");
  return sarray;
}
...
alurusortint(void *space, Uint *s, Uint *l) {
  ...
  fprintf(stderr, "alurusortint: classify int\n");
  cl = classifyint(space, s, len, &noL, &noS);
  
  fprintf(stderr, "alurusortint: getting bit\n");
  if(!getbit(cl, (Uint) len-1) && noS ==1) {

    fprintf(stderr, "alurusortint: aluruSuffixArraySint\n");
    /*printf("fewintS\n");*/
  ...
  fprintf(stderr, "alurusortint: init bcktsA\n");
  bcktsA = initbitarray(space, len);
  fprintf(stderr, "alurusortint: countingsort\n");
  A=countingsortint(space, s, len, bcktsA);

  /*sort type S suffixes*/
  if(!getbit(cl, (Uint) len-1)) {
   
    fprintf(stderr, "alurusortint: Sorting type S suffixes. Init bcktsB.\n");
    fprintf(stderr, "%d\t%d\t%d\n\n", noS, noL, len);
  
    bcktsB = initbitarray(bcktsB, noS);
    B = arrayB(space, A, len, noS, bcktsA, bcktsB, cl, 0);
    fprintf(stderr, "alurusortint: enter Qmaxdist\n");
    maxdist = Qmaxdist(space, cl, len, 0);

    fprintf(stderr, "alurusortint: enter Qdist\n");
    dist = Qdist(space, cl, len, 0);


    fprintf(stderr, "alurusortint: enter distCount\n");
    ...
    if(valbitarray(bcktsB, noS, 1)) {
      ...
      fprintf(stderr, "alurusortint: valbitarraysortedS.\n");
      A = aluruSuffixArraySint(space, s, len, B, noS, cl);
    ...
    fprintf(stderr,"alurusortint: enter tprime\n");
    tprime = Tprime(space, len, B, noS, bcktsB, cl, 0);
    FREEMEMORY(space, B);
    FREEMEMORY(space, bcktsB);

    fprintf(stderr,"alurusortint: enter alurusortint\n");
    B = alurusortint(space, tprime, &noS);
    FREEMEMORY(space, tprime);
    
    fprintf(stderr, "reconstructintS\n");
    ...
    *l = len;
    ...
    fprintf(stderr, "alurusortint: enter get listsL\n");
    ...
    fprintf(stderr, "alurusortint: sort listsL\n");
    ...
    if(valbitarray(bcktsB, noL, 1)) {
      ...
      FREEMEMORY(space, bcktsB);
      fprintf(stderr, "alurusortint: valbitarraysortedL.\n");
    ...
    fprintf(stderr, "reconstructintL\n");
...
alurusort(void *space, char *s, Uint *l) {
  ...
  fprintf(stderr, "alurusort: classify\n");
  cl = classify(space, s, len, &noL, &noS);
  
  fprintf(stderr, "alurusort: getting bit\n");
  if(!getbit(cl, (Uint) len-1) && noS ==1) {
    fprintf(stderr, "alurusort: fewcharS\n");
    B = ALLOCMEMORY(space, NULL, Uint, 1);
    B[0] = len-1;
  ...
  if(!getbit(cl, (Uint) len-1)) {
    ...
    fprintf(stderr, "not bit alurusort: alloc B of size %u\n", noS);
    B = ALLOCMEMORY(space, NULL, Uint, noS);

    fprintf(stderr, "alurusort: initbitarray of size %u\n", noS);
    bckts = initbitarray(bckts, noS);

    fprintf(stderr, "alurusort: Qmaxdist in cl of size %u\n", len);
    dist = Qmaxdist(space, cl, len, 0);

    fprintf(stderr, "alurusort: scan B\n");
    
    j=0;
    for(i=0; i < len; i++) {
      B[j] = i;
      if (j > len) fprintf(stderr, "%u > %u\n", j, len);
      j +=  (!getbit(cl, (Uint) i)) ? 1 : 0;
    }
    
    fprintf(stderr, "alurusort: substringsort\n");
    B = substringsort(space, s, B, cl, len, bckts, noS, dist, 0);

    fprintf(stderr, "checking valbitarray\n");
    if(valbitarray(bckts, noS, 1)) {

      FREEMEMORY(space, bckts);
      /*printf("valbitarraysortedcharS.\n");*/

      fprintf(stderr, "aluruSuffixArrayS start (if cond 1)\n");
      A = aluruSuffixArrayS(space, s, len, B, noS, cl);
    ...
    fprintf(stderr,"enter Tprime calculation");
    tprime = Tprime(space, len, B, noS, bckts, cl, 0);
    FREEMEMORY(space, B);
    FREEMEMORY(space, bckts);
    
    fprintf(stderr, "enter alursortint \n");
    B = alurusortint(space, tprime, &noS);
    FREEMEMORY(space, tprime);
    ...
    fprintf(stderr, "enter aluruSuffixArrayS start\n");   
    A = aluruSuffixArrayS(space, s, len, B, noS, cl);
    FREEMEMORY(space, B);
    FREEMEMORY(space, cl);
    ...
    *l = len;
    ...
    fprintf(stderr, "bit alurusort: alloc B of size %u\n", noL);
    ...
    fprintf(stderr, "enter reconstruction\n");
    reconstruct(space, len, B, noL, cl, 1);
    fprintf(stderr, "enter aluruSuffixArrayL start\n");
    A = aluruSuffixArrayL(space, s, len, B, noL, cl);
